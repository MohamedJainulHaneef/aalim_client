...
import { faSave, faTimes, faUser, faLock, faIdBadge, faUserEdit } from '@fortawesome/free-solid-svg-icons';
import { useEdit } from '../../../hooks/useEdit';
const apiUrl = import.meta.env.VITE_API_URL;

function EditPopup({ onSuccess, onClose, selectedUser }) {
    ...
    const Url = `${apiUrl}/api/users/editStaff`;
    ...
    const handleSubmit = async () => {
        ...
        const data = await editData(Url, formData);
        if (data) {
            onSuccess?.();
            onClose?.();
        }
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                            <div className="bg-white/20 p-2 rounded-lg">
                                <FontAwesomeIcon icon={faUserEdit} className="text-white text-lg" />
                            </div>
                            ...
                            <div key={field.name}>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <FontAwesomeIcon
                                            icon={field.icon}
                                            className={`${field.readOnly ? 'text-gray-300' : 'text-gray-400'}`}
                                        />
                                    </div>
                                    <input
                                        type={field.type}
                                ...
                                {touched[field.name] && fieldErrors[field.name] && !field.readOnly && (
