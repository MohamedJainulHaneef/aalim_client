...
import { faEdit, faTrash, faGraduationCap } from '@fortawesome/free-solid-svg-icons';
...
function AcademicTable() {
    ...
    useEffect(() => {
        fetchData(`${apiUrl}/api/academic/academicInfo`);
    }, [fetchData]);
    ...
    const sortedData = data ? [...data].sort((a, b) => new Date(a.academicFromDate) - new Date(b.academicFromDate)) : [];
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100 text-sm">
                            {sortedData.length > 0 ? (
                                    ...
                                    <tr key={details._id} className="hover:bg-slate-50/60 transition-colors duration-150">
                                        <td className="px-6 py-3.5 text-left">
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-lg flex items-center justify-center text-xs font-bold shadow-2xs select-none bg-indigo-100 text-indigo-600">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="font-semibold text-slate-800 tracking-normal text-sm">
                                                        {formatDate(details.academicFromDate)} - {formatDate(details.academicToDate)}
                                                    </span>
                                                </div>
                                            </div>
                                        </td>
                                ...
                                <tr>
                                    <td colSpan={4} className="py-12 text-center text-slate-400 font-medium tracking-normal">
